...
OUTPUT_DIR = r"D:\SMV_Data"           # Carpeta raiz donde se guardara todo
...
EMPRESAS = [
    ...
    "Pacifico Vida",
    "Administradora Jockey Plaza Shopping Center S.A.",  # <-- ya confirmada con exito
...
# Rango reducido usado en las pruebas anteriores (ya no activo)
# ANIOS = [2023, 1999, 2026]

TIPOS = ["Individual", "Consolidada"]  # Paso 1: ya probamos Individual solo, ahora agregamos Consolidada
...
CHROME_PROFILE_DIR = os.path.join(OUTPUT_DIR, "_chrome_profile")  # perfil/cache de Chrome, tambien en D:
os.environ["TMP"] = os.path.join(OUTPUT_DIR, "_temp")    # FORZAR (no setdefault): Windows ya trae
os.environ["TEMP"] = os.path.join(OUTPUT_DIR, "_temp")   # TMP/TEMP definidos apuntando a C:, y
                                                            # setdefault() no los habria sobreescrito
...
def setup_driver():
    ...
    y archivos temporales) se redirige a D:, para no usar nada de espacio en C:."""
    ...
    # IMPORTANTE: perfil/cache completo de Chrome en D:, no en C:
...
def buscar_empresa(driver, nombre_empresa):
    ...
    # Intentos quitando palabras desde el INICIO (útil cuando la palabra distintiva
    # está al final, ej. "La Positiva Seguros..." -> "Positiva Seguros...")
    intentos_desde_inicio = [" ".join(palabras[i:]) for i in range(len(palabras) - 1)]
    # Intentos quitando palabras desde el FINAL (útil cuando la palabra distintiva
    # está al principio, ej. "Cencosud Shopping S.A." -> "Cencosud Shopping" -> "Cencosud")
